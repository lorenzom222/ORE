...
"""
python3 main_swav_mobilenet.py --data_path /home/nina/ore_balanced_dataset --max-class 1960
"""

logger = getLogger()

parser = argparse.ArgumentParser(description="Implementation of SwAV + MobileNetV3L")
...
#########################
parser.add_argument(
    "--arch", default="mobilenet_v3_large", type=str, help="convnet architecture"
)
parser.add_argument(
    "--hidden_mlp",
    default=960,
...
parser.add_argument(
    "--checkpoint_freq", type=int, default=25, help="Save the model periodically"
)
parser.add_argument(
    "--use_fp16",
    type=bool_flag,
...
parser.add_argument(
    "--max-class", default=100, type=int, help="number of classes used for training"
)
...
def main():
    ...
    #### Filtering classes for SLDA pre-training #####################################################
    # if args.max_class != 1000:
    # filter dataset
    # train_labels_file = np.load('./imagenet_ordering_labels/imagenet_train_labels.npy')
    # curr_idx = filter_by_class(train_labels_file, min_class=0, max_class=args.max_class)
    # train_dataset = torch.utils.data.Subset(train_dataset, curr_idx)
    ##################################################################################################
    ...
    # build model
    # model = resnet_models.__dict__['resnet18'](
    #    normalize=True,
    #    hidden_mlp=args.hidden_mlp,
    #    output_dim=args.feat_dim,
    #    nmb_prototypes=args.nmb_prototypes,
    # )
    model = mobilenet_models.__dict__[args.arch](pretrained=False)
    # print(model)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        # train the network
        scores, queue, loss_avg = train(
            logger, train_loader, model, optimizer, epoch, lr_schedule, queue
        )
        ...
        if args.rank == 0:
            ...
            if epoch % args.checkpoint_freq == 0 or epoch == args.epochs - 1:
                np.save(os.path.join(args.dump_path, "swav_train_loss.npy"), loss_epc)
                shutil.copyfile(
                    os.path.join(args.dump_path, "checkpoint.pth.tar"),
                    os.path.join(
                        args.dump_checkpoints, "SwAV_MobNet_Ckpt_" + str(epoch) + ".pth"
                    ),
                )
...
def train(logger, train_loader, model, optimizer, epoch, lr_schedule, queue):
    ...
    for it, inputs in tqdm(
        enumerate(train_loader), total=len(train_loader), desc="Epoch"
    ):
    ...
    return (epoch, losses.avg), queue, losses.avg
