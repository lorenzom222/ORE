...
class ResNet(nn.Module):
    def __init__(
        ...
        self.conv1 = Conv2d(3, num_out_filters, kernel_size=7, stride=2, padding=2, bias=False)
        #self.bn1 = norm_layer(num_out_filters)
        self.bn1 = nn.GroupNorm(32, num_out_filters)
        self.relu = nn.ReLU(inplace=True)
        ...
        if output_dim == 0:
            ...
            #print('number of out filters:', num_out_filters)
            self.projection_head = nn.Sequential(
                nn.Linear(hidden_mlp, hidden_mlp),
                #nn.Linear(num_out_filters * block.expansion, hidden_mlp),
                nn.BatchNorm1d(hidden_mlp),
                #nn.GroupNorm(32, hidden_mlp),
                nn.ReLU(inplace=True),
                nn.Linear(hidden_mlp, output_dim),
            )
            #m = self.projection_head[1]  ## added by me
            #assert isinstance(m, nn.GroupNorm)
            #gn_init(m)
...
#RESNET18
#def resnet18(**kwargs):
#    return ResNet(BasicBlock, [2, 2, 2, 2], **kwargs) # entire ResNet18

def resnet18(**kwargs):
    return ResNet(BasicBlock, [2, 2, 2, 1], **kwargs) # First 15 ResNet18 layers
...
# def resnet18w5(**kwargs):
#     return ResNet(BasicBlock, [2, 2, 2, 2], widen=5, **kwargs)
