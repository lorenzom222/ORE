...
from .utils import _make_divisible #can't solve mobilnetv2 import problem
from typing import Any, Callable, Dict, Optional, Tuple, TypeVar, Union
# from timm.models.layers import trunc_normal_
from .utils import trunc_normal_ #can't solve timm import problem

#### This scripts is MobileNet with Linear FC layer ####
...
class MobileNetV3(nn.Module):
    ...
    def __init__(
        ...
        print("Number of classes in output layer:", num_classes)
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        x = self.classifier(x) # N x 1280 >> raw features without activation
...
class MobNet_StartAt_Layer8(nn.Module):
    def __init__(self, num_classes=None, pretrain_classes=None):
        super(MobNet_StartAt_Layer8, self).__init__()

        self.model = mobilenet_v3_large(pretrained=False, num_classes=pretrain_classes)
...
# net = mobilenet_v3_large(pretrained=False, width_mult=1.0, reduced_tail=False, dilated=False)
# print(net)
# input = torch.randn((10, 3, 224, 224), dtype=torch.float32)
# output = net(input)
# print('shape of output:', output.shape)

# #Test changing classifier layer
# # net = MobNet_StartAt_Layer8(num_classes=30)
# net = eval('MobNet_StartAt_Layer8')(num_classes=30)

# # net = eval('MobNet_StartAt_Layer8')()
# for param_tensor in net.state_dict():
#    print(param_tensor, "\t", net.state_dict()[param_tensor].size())
# print(net)
# net.requires_grad_(False)
# net.model.classifier[3].requires_grad_(True)
# classifier_parameters, frozen_parameters = [], []
# for k, param in net.named_parameters():
#    if k.startswith('model.classifier.3.weight'):
...
#        #print(k, "\t", param.size())
# param_groups = [dict(params=classifier_parameters, lr=0.1)]

# criterion = nn.CrossEntropyLoss()
# optimizer = optim.SGD(net.parameters(), lr=0.1, momentum=0.9)
# optimizer = optim.SGD(param_groups, lr=0.1, momentum=0.9)
# inputs = torch.randn((4, 3, 224, 224), dtype=torch.float32)
# inputs = torch.randn((4, 80, 14, 14), dtype=torch.float32)
# labels = torch.div(torch.rand(4)*10, 5, rounding_mode='floor')
# print(labels)
# # zero_grad
# optimizer.zero_grad()
# outputs = net(inputs)
